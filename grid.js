...
function createGrid() {
  ...
  for(let i = 1; i <= gridSize; i++) {
    let id = "n" + i;
    let div = document.getElementById("target").appendChild(document.createElement("div"));
...
function toggle(position) {
  ...
  let prevElement = document.getElementById("n" + prev);

  if (position > gridSize) return;
    element.classList.toggle('on');
    if(prev > 0) {
        prevElement.classList.toggle('on');
    }
}
